...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions: # iterates through all possible actions for the problem
            new_node = PgNode_a(action)
            prerequisites = new_node.prenodes
            if prerequisites.issubset(previous_s): # checks if prerequisites of current action hold in the previous state
                self.a_levels[level].add(new_node)
                for s_node in self.s_levels[level]:
                    if s_node in new_node.prenodes:
                        s_node.children.add(new_node) # adds the action node to the children of the previous state node
                        new_node.parents.add(s_node) # adds the previous state node to the parents of the action node
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level-1]:
            for s_node in action_node.effnodes:
                self.s_levels[level].add(s_node) # adds an effect (s_node) of the current action node to the new S level
                
                s_node.parents.add(action_node) # adds the previous action node to the parents of the state node
                action_node.children.add(s_node) # adds the state node to the children of the previous action node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        precond_a1 = action_a1.precond_pos
        precond_a2 = action_a2.precond_pos
        
        effect_a1 = action_a1.effect_add
        effect_a2 = action_a2.effect_add
        
        if effect_a1 == precond_a2: # checks if the effect of Action Node 1 results in the precondition of Action Node 2 (i.e. negating the effect of Action Node 2)
            isInconsistent = True
        if effect_a2 == precond_a1: # checks if the effect of Action Node 2 results in the precondition of Action Node 1 (i.e. negating the effect of Action Node 1)
            isInconsistent = True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if not node_a1.prenodes and not node_a2.prenodes: # returns False if there are no preconditions for both Action Nodes
            return False
        
        action_a1 = node_a1.action
        action_a2 = node_a2.action
        
        precond1_a1 = action_a1.precond_pos
        precond1_a2 = action_a2.precond_pos
        
        precond2_a1 = action_a1.precond_neg
        precond2_a2 = action_a2.precond_neg
        
        effect1_a1 = action_a1.effect_rem
        effect1_a2 = action_a2.effect_rem
        
        effect2_a1 = action_a1.effect_add
        effect2_a2 = action_a2.effect_add
        
        if effect1_a1 == precond1_a2: # checks if the effect of Action Node 1 removes a positive precondition of Action Node 2
            isInterference = True
        if effect1_a2 == precond1_a1: # checks if the effect of Action Node 2 removes a positive precondition of Action Node 1
            isInterference = True
        if effect2_a1 == precond2_a2: # checks if the effect of Action Node 1 adds a negative precondition of Action Node 2
            isInterference = True
        if effect2_a2 == precond2_a1: # checks if the effect of Action Node 2 adds a negative precondition of Action Node 1
            isInterference = True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for i in parents_a1:
            for j in parents_a2:
                if i.is_mutex(j): # checks if parents are mutex
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for i in parents_s1: 
            for j in parents_s2:
                if i.is_mutex(j) == False: # if a pair of parents is not mutex, return False immediately
                    return False
        
        return True # return True if all parents are mutex
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            isFound = False
            i = 0
            while isFound != True: # iterates until the first level of the current goal is found
                for j in self.s_levels[i]:
                    if goal == j.symbol and j.is_pos == True: # adds the level for the current goal if it matches a positive fluent in the current state of the level
                        level_sum+=i
                        isFound = True
                i+=1
        return level_sum
